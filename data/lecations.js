export const locations = [
  {
    region: 'Dhaka',
    district: 'Dhaka',
    city: 'Dhaka',
    covered_area: ['Uttara', 'Dhanmondi', 'Mirpur', 'Mohammadpur'],
    status: 'active',
    longitude: 90.4125,
    latitude: 23.8103,
    cinemas: ['Bosondora', 'Jamuna', 'Star Cineplex', 'Blockbuster'],
  },
  {
    region: 'Dhaka',
    district: 'Faridpur',
    city: 'Faridpur',
    covered_area: ['Goalanda', 'Boalmari', 'Bhanga'],
    status: 'active',
    flowchart: 'https://example.com/faridpur-flowchart.png',
    longitude: 89.8333,
    latitude: 23.6,
    cinemas: ['Faridpur Hall', 'Local Cineplex'],
  },
  {
    region: 'Dhaka',
    district: 'Gazipur',
    city: 'Gazipur',
    covered_area: ['Tongi', 'Kaliakair', 'Sreepur'],
    status: 'active',
    flowchart: 'https://example.com/gazipur-flowchart.png',
    longitude: 90.4203,
    latitude: 23.9999,
    cinemas: ['Gazipur Star', 'Tongi Cinema'],
  },
  {
    region: 'Dhaka',
    district: 'Gopalganj',
    city: 'Gopalganj',
    covered_area: ['Tungipara', 'Kotalipara', 'Kashiani'],
    status: 'active',
    flowchart: 'https://example.com/gopalganj-flowchart.png',
    longitude: 89.8266,
    latitude: 23.0052,
    cinemas: ['Gopalganj Hall', 'River Cineplex'],
  },
  {
    region: 'Dhaka',
    district: 'Kishoreganj',
    city: 'Kishoreganj',
    covered_area: ['Bajitpur', 'Kuliarchar', 'Pakundia'],
    status: 'active',
    flowchart: 'https://example.com/kishoreganj-flowchart.png',
    longitude: 90.7829,
    latitude: 24.426,
    cinemas: ['Kishoreganj Cinema', 'Starlight Hall'],
  },
  {
    region: 'Dhaka',
    district: 'Madaripur',
    city: 'Madaripur',
    covered_area: ['Rajoir', 'Kalkini', 'Shibchar'],
    status: 'active',
    flowchart: 'https://example.com/madaripur-flowchart.png',
    longitude: 90.2,
    latitude: 23.17,
    cinemas: ['Madaripur Plaza', 'River View Cine'],
  },
  {
    region: 'Dhaka',
    district: 'Manikganj',
    city: 'Manikganj',
    covered_area: ['Saturia', 'Shivalaya', 'Ghior'],
    status: 'active',
    flowchart: 'https://example.com/manikganj-flowchart.png',
    longitude: 89.9767,
    latitude: 23.8617,
    cinemas: ['Manikganj Star', 'Central Cinema'],
  },
  {
    region: 'Dhaka',
    district: 'Munshiganj',
    city: 'Munshiganj',
    covered_area: ['Sreenagar', 'Lohajang', 'Sirajdikhan'],
    status: 'active',
    flowchart: 'https://example.com/munshiganj-flowchart.png',
    longitude: 90.5305,
    latitude: 23.55,
    cinemas: ['Munshiganj Cineplex', 'Sreenagar Hall'],
  },
  {
    region: 'Dhaka',
    district: 'Narayanganj',
    city: 'Narayanganj',
    covered_area: ['Fatullah', 'Siddhirganj', 'Rupganj'],
    status: 'active',
    flowchart: 'https://example.com/narayanganj-flowchart.png',
    longitude: 90.5,
    latitude: 23.62,
    cinemas: ['Narayanganj Star', 'Rupganj Cinema'],
  },
  {
    region: 'Dhaka',
    district: 'Narsingdi',
    city: 'Narsingdi',
    covered_area: ['Palash', 'Belabo', 'Raipura'],
    status: 'active',
    flowchart: 'https://example.com/narsingdi-flowchart.png',
    longitude: 90.7156,
    latitude: 23.9226,
    cinemas: ['Narsingdi Hall', 'Palash Cineplex'],
  },
  {
    region: 'Dhaka',
    district: 'Rajbari',
    city: 'Rajbari',
    covered_area: ['Pangsha', 'Kalukhali', 'Baliakandi'],
    status: 'active',
    flowchart: 'https://example.com/rajbari-flowchart.png',
    longitude: 89.65,
    latitude: 23.7576,
    cinemas: ['Rajbari Cinema', 'Pangsha Hall'],
  },
  {
    region: 'Dhaka',
    district: 'Shariatpur',
    city: 'Shariatpur',
    covered_area: ['Zajira', 'Naria', 'Gosairhat'],
    status: 'active',
    flowchart: 'https://example.com/shariatpur-flowchart.png',
    longitude: 90.4308,
    latitude: 23.22,
    cinemas: ['Shariatpur Star', 'Naria Cine'],
  },
  {
    region: 'Dhaka',
    district: 'Tangail',
    city: 'Tangail',
    covered_area: ['Delduar', 'Ghatail', 'Kalihati'],
    status: 'active',
    flowchart: 'https://example.com/tangail-flowchart.png',
    longitude: 89.92,
    latitude: 24.25,
    cinemas: ['Tangail Cineplex', 'Kalihati Hall'],
  },
  {
    region: 'Chattogram',
    district: 'Chattogram',
    city: 'Chattogram',
    covered_area: [
      'Pahartali',
      'Kotwali',
      'Halishahar',
      'Panchlaish',
      'Agrabad',
      'Chandgaon',
    ],
    status: 'active',
    flowchart: 'https://example.com/chattogram-flowchart.png',
    longitude: 91.8123,
    latitude: 22.3569,
    cinemas: ['Chittagong Plaza', 'Pahartali Cine', 'Star Cineplex'],
  },
  {
    region: 'Chattogram',
    district: "Cox's Bazar",
    city: "Cox's Bazar",
    covered_area: ['Teknaf', 'Ukhia', 'Chakaria', 'Ramu'],
    status: 'active',
    flowchart: 'https://example.com/coxbazar-flowchart.png',
    longitude: 92.0165,
    latitude: 21.4272,
    cinemas: ["Cox's Bazar Star", 'Beach View Cinema'],
  },
  {
    region: 'Chattogram',
    district: 'Cumilla',
    city: 'Cumilla',
    covered_area: ['Laksam', 'Debidwar', 'Chandina', 'Muradnagar'],
    status: 'active',
    flowchart: 'https://example.com/cumilla-flowchart.png',
    longitude: 91.1809,
    latitude: 23.4573,
    cinemas: ['Cumilla Cineplex', 'Laksam Hall'],
  },
  {
    region: 'Chattogram',
    district: 'Brahmanbaria',
    city: 'Brahmanbaria',
    covered_area: ['Nabinagar', 'Ashuganj', 'Sarail'],
    status: 'active',
    flowchart: 'https://example.com/brahmanbaria-flowchart.png',
    longitude: 91.1116,
    latitude: 23.9571,
    cinemas: ['Brahmanbaria Star', 'Ashuganj Cinema'],
  },
  {
    region: 'Chattogram',
    district: 'Chandpur',
    city: 'Chandpur',
    covered_area: ['Haimchar', 'Matlab', 'Shahrasti'],
    status: 'active',
    flowchart: 'https://example.com/chandpur-flowchart.png',
    longitude: 90.85,
    latitude: 23.2333,
    cinemas: ['Chandpur Hall', 'Matlab Cine'],
  },
  {
    region: 'Chattogram',
    district: 'Feni',
    city: 'Feni',
    covered_area: ['Parshuram', 'Daganbhuiyan', 'Chhagalnaiya'],
    status: 'active',
    flowchart: 'https://example.com/feni-flowchart.png',
    longitude: 91.4,
    latitude: 23.0167,
    cinemas: ['Feni Star', 'Parshuram Cinema'],
  },
  {
    region: 'Chattogram',
    district: 'Khagrachari',
    city: 'Khagrachari',
    covered_area: ['Ramgarh', 'Mahalchari', 'Dighinala'],
    status: 'active',
    flowchart: 'https://example.com/khagrachari-flowchart.png',
    longitude: 91.9667,
    latitude: 23.1,
    cinemas: ['Khagrachari Cine', 'Hill View Hall'],
  },
  {
    region: 'Chattogram',
    district: 'Lakshmipur',
    city: 'Lakshmipur',
    covered_area: ['Raipur', 'Ramganj', 'Kamalnagar'],
    status: 'active',
    flowchart: 'https://example.com/lakshmipur-flowchart.png',
    longitude: 90.8415,
    latitude: 22.9444,
    cinemas: ['Lakshmipur Star', 'Raipur Cinema'],
  },
  {
    region: 'Chattogram',
    district: 'Noakhali',
    city: 'Noakhali',
    covered_area: ['Begumganj', 'Senbagh', 'Chatkhil'],
    status: 'active',
    flowchart: 'https://example.com/noakhali-flowchart.png',
    longitude: 91.0995,
    latitude: 22.8245,
    cinemas: ['Noakhali Cineplex', 'Senbagh Hall'],
  },
  {
    region: 'Chattogram',
    district: 'Rangamati',
    city: 'Rangamati',
    covered_area: ['Baghaichhari', 'Kaptai', 'Juraichhari'],
    status: 'active',
    flowchart: 'https://example.com/rangamati-flowchart.png',
    longitude: 92.2,
    latitude: 22.65,
    cinemas: ['Rangamati Star', 'Kaptai Cinema'],
  },
  {
    region: 'Sylhet',
    district: 'Sylhet',
    city: 'Sylhet',
    covered_area: [
      'Zindabazar',
      'Ambarkhana',
      'Dargah Gate',
      'South Surma',
      'Subid Bazar',
      'Tilagor',
    ],
    status: 'active',
    flowchart: 'https://example.com/sylhet-flowchart.png',
    longitude: 91.8662,
    latitude: 24.8949,
    cinemas: ['Sylhet Star', 'Tea Garden Hall', 'Zindabazar Cine'],
  },
  {
    region: 'Sylhet',
    district: 'Moulvibazar',
    city: 'Moulvibazar',
    covered_area: ['Sreemangal', 'Kamalganj', 'Kulaura', 'Barlekha'],
    status: 'active',
    flowchart: 'https://example.com/moulvibazar-flowchart.png',
    longitude: 91.7832,
    latitude: 24.4826,
    cinemas: ['Moulvibazar Cine', 'Sreemangal Hall'],
  },
  {
    region: 'Sylhet',
    district: 'Habiganj',
    city: 'Habiganj',
    covered_area: ['Shaistaganj', 'Madhabpur', 'Chunarughat', 'Nabiganj'],
    status: 'active',
    flowchart: 'https://example.com/habiganj-flowchart.png',
    longitude: 91.4026,
    latitude: 24.3745,
    cinemas: ['Habiganj Star', 'Madhabpur Cinema'],
  },
  {
    region: 'Sylhet',
    district: 'Sunamganj',
    city: 'Sunamganj',
    covered_area: ['Jagannathpur', 'Chhatak', 'Tahirpur', 'Dowarabazar'],
    status: 'active',
    flowchart: 'https://example.com/sunamganj-flowchart.png',
    longitude: 91.395,
    latitude: 25.0658,
    cinemas: ['Sunamganj Cineplex', 'Jagannathpur Hall'],
  },
  {
    region: 'Rangpur',
    district: 'Rangpur',
    city: 'Rangpur',
    covered_area: [
      'Jahaj Company',
      'Pairaband',
      'Mahiganj',
      'Satmatha',
      'Lalbagh',
    ],
    status: 'active',
    flowchart: 'https://example.com/rangpur-flowchart.png',
    longitude: 89.2752,
    latitude: 25.746,
    cinemas: ['Rangpur Star', 'Mahiganj Cinema'],
  },
  {
    region: 'Rangpur',
    district: 'Dinajpur',
    city: 'Dinajpur',
    covered_area: ['Birampur', 'Fulbari', 'Parbatipur', 'Nawabganj'],
    status: 'active',
    flowchart: 'https://example.com/dinajpur-flowchart.png',
    longitude: 88.6414,
    latitude: 25.6275,
    cinemas: ['Dinajpur Cineplex', 'Fulbari Hall'],
  },
  {
    region: 'Rangpur',
    district: 'Thakurgaon',
    city: 'Thakurgaon',
    covered_area: ['Pirganj', 'Ranisankail', 'Baliadangi'],
    status: 'active',
    flowchart: 'https://example.com/thakurgaon-flowchart.png',
    longitude: 88.466,
    latitude: 26.0333,
    cinemas: ['Thakurgaon Star', 'Pirganj Cinema'],
  },
  {
    region: 'Rangpur',
    district: 'Panchagarh',
    city: 'Panchagarh',
    covered_area: ['Tetulia', 'Boda', 'Atwari'],
    status: 'active',
    flowchart: 'https://example.com/panchagarh-flowchart.png',
    longitude: 88.5658,
    latitude: 26.3411,
    cinemas: ['Panchagarh Cine', 'Tetulia Hall'],
  },
  {
    region: 'Rangpur',
    district: 'Nilphamari',
    city: 'Nilphamari',
    covered_area: ['Saidpur', 'Domar', 'Jaldhaka'],
    status: 'active',
    flowchart: 'https://example.com/nilphamari-flowchart.png',
    longitude: 88.856,
    latitude: 25.931,
    cinemas: ['Nilphamari Star', 'Saidpur Cinema'],
  },
  {
    region: 'Rangpur',
    district: 'Lalmonirhat',
    city: 'Lalmonirhat',
    covered_area: ['Hatibandha', 'Patgram', 'Aditmari'],
    status: 'active',
    flowchart: 'https://example.com/lalmonirhat-flowchart.png',
    longitude: 89.1662,
    latitude: 25.9167,
    cinemas: ['Lalmonirhat Cineplex', 'Patgram Hall'],
  },
  {
    region: 'Rangpur',
    district: 'Kurigram',
    city: 'Kurigram',
    covered_area: ['Nageshwari', 'Bhurungamari', 'Chilmari', 'Ulipur'],
    status: 'active',
    flowchart: 'https://example.com/kurigram-flowchart.png',
    longitude: 89.65,
    latitude: 25.8054,
    cinemas: ['Kurigram Star', 'Ulipur Cinema'],
  },
  {
    region: 'Rangpur',
    district: 'Gaibandha',
    city: 'Gaibandha',
    covered_area: ['Gobindaganj', 'Sundarganj', 'Palashbari', 'Phulchhari'],
    status: 'active',
    flowchart: 'https://example.com/gaibandha-flowchart.png',
    longitude: 89.5418,
    latitude: 25.3288,
    cinemas: ['Gaibandha Cine', 'Gobindaganj Hall'],
  },
  {
    region: 'Khulna',
    district: 'Khulna',
    city: 'Khulna',
    covered_area: [
      'Sonadanga',
      'Khalishpur',
      'Daulatpur',
      'Shib Bari',
      'Boyra',
    ],
    status: 'active',
    flowchart: 'https://example.com/khulna-flowchart.png',
    longitude: 89.5672,
    latitude: 22.8456,
    cinemas: ['Khulna Star', 'Sonadanga Cineplex', 'Shib Bari Hall'],
  },
  {
    region: 'Khulna',
    district: 'Jessore',
    city: 'Jessore',
    covered_area: ['Chowgachha', 'Bagharpara', 'Manirampur', 'Abhaynagar'],
    status: 'active',
    flowchart: 'https://example.com/jessore-flowchart.png',
    longitude: 89.2167,
    latitude: 23.17,
    cinemas: ['Jessore Cinema', 'Manirampur Hall'],
  },
  {
    region: 'Khulna',
    district: 'Satkhira',
    city: 'Satkhira',
    covered_area: ['Tala', 'Assasuni', 'Kalaroa', 'Debhata'],
    status: 'active',
    flowchart: 'https://example.com/satkhira-flowchart.png',
    longitude: 89.0809,
    latitude: 22.7085,
    cinemas: ['Satkhira Star', 'Tala Cine'],
  },
  {
    region: 'Khulna',
    district: 'Bagerhat',
    city: 'Bagerhat',
    covered_area: ['Mongla', 'Rampal', 'Fakirhat', 'Kachua'],
    status: 'active',
    flowchart: 'https://example.com/bagerhat-flowchart.png',
    longitude: 89.7926,
    latitude: 22.6516,
    cinemas: ['Bagerhat Cineplex', 'Mongla Hall'],
  },
  {
    region: 'Khulna',
    district: 'Magura',
    city: 'Magura',
    covered_area: ['Sreepur', 'Mohammadpur', 'Shalikha'],
    status: 'active',
    flowchart: 'https://example.com/magura-flowchart.png',
    longitude: 89.4194,
    latitude: 23.4853,
    cinemas: ['Magura Star', 'Sreepur Cinema'],
  },
  {
    region: 'Khulna',
    district: 'Narail',
    city: 'Narail',
    covered_area: ['Lohagara', 'Kalia', 'Narail Sadar'],
    status: 'active',
    flowchart: 'https://example.com/narail-flowchart.png',
    longitude: 89.5,
    latitude: 23.1667,
    cinemas: ['Narail Cine', 'Lohagara Hall'],
  },
  {
    region: 'Khulna',
    district: 'Jhenaidah',
    city: 'Jhenaidah',
    covered_area: ['Harinakunda', 'Shailkupa', 'Kaliganj'],
    status: 'active',
    flowchart: 'https://example.com/jhenaidah-flowchart.png',
    longitude: 89.1833,
    latitude: 23.5333,
    cinemas: ['Jhenaidah Star', 'Kaliganj Cinema'],
  },
  {
    region: 'Khulna',
    district: 'Chuadanga',
    city: 'Chuadanga',
    covered_area: ['Alamdanga', 'Damurhuda', 'Jibannagar'],
    status: 'active',
    flowchart: 'https://example.com/chuadanga-flowchart.png',
    longitude: 88.85,
    latitude: 23.64,
    cinemas: ['Chuadanga Cineplex', 'Alamdanga Hall'],
  },
  {
    region: 'Khulna',
    district: 'Meherpur',
    city: 'Meherpur',
    covered_area: ['Mujibnagar', 'Gangni'],
    status: 'active',
    flowchart: 'https://example.com/meherpur-flowchart.png',
    longitude: 88.6318,
    latitude: 23.7623,
    cinemas: ['Meherpur Star', 'Gangni Cinema'],
  },
  {
    region: 'Rajshahi',
    district: 'Rajshahi',
    city: 'Rajshahi',
    covered_area: ['Boalia', 'Rajpara', 'Motihar', 'Shah Makhdum', 'Paba'],
    status: 'active',
    flowchart: 'https://example.com/rajshahi-flowchart.png',
    longitude: 88.6087,
    latitude: 24.3745,
    cinemas: ['Rajshahi Star', 'Boalia Cineplex', 'Motihar Hall'],
  },
  {
    region: 'Rajshahi',
    district: 'Natore',
    city: 'Natore',
    covered_area: ['Baraigram', 'Bagatipara', 'Lalpur', 'Singra'],
    status: 'active',
    flowchart: 'https://example.com/natore-flowchart.png',
    longitude: 89,
    latitude: 24.4167,
    cinemas: ['Natore Cine', 'Lalpur Hall'],
  },
  {
    region: 'Rajshahi',
    district: 'Naogaon',
    city: 'Naogaon',
    covered_area: ['Manda', 'Sapahar', 'Porsha', 'Patnitala'],
    status: 'active',
    flowchart: 'https://example.com/naogaon-flowchart.png',
    longitude: 88.93,
    latitude: 24.8236,
    cinemas: ['Naogaon Star', 'Sapahar Cinema'],
  },
  {
    region: 'Rajshahi',
    district: 'Chapainawabganj',
    city: 'Chapainawabganj',
    covered_area: ['Shibganj', 'Bholahat', 'Gomostapur'],
    status: 'active',
    flowchart: 'https://example.com/chapai-flowchart.png',
    longitude: 88.27,
    latitude: 24.5962,
    cinemas: ['Chapainawabganj Cine', 'Shibganj Hall'],
  },
  {
    region: 'Rajshahi',
    district: 'Pabna',
    city: 'Pabna',
    covered_area: ['Ishwardi', 'Bera', 'Chatmohar', 'Atgharia'],
    status: 'active',
    flowchart: 'https://example.com/pabna-flowchart.png',
    longitude: 89.2331,
    latitude: 24.0037,
    cinemas: ['Pabna Star', 'Ishwardi Cinema'],
  },
  {
    region: 'Rajshahi',
    district: 'Sirajganj',
    city: 'Sirajganj',
    covered_area: ['Ullapara', 'Kazipur', 'Shahjadpur', 'Belkuchi'],
    status: 'active',
    flowchart: 'https://example.com/sirajganj-flowchart.png',
    longitude: 89.7167,
    latitude: 24.45,
    cinemas: ['Sirajganj Cineplex', 'Shahjadpur Hall'],
  },
  {
    region: 'Rajshahi',
    district: 'Joypurhat',
    city: 'Joypurhat',
    covered_area: ['Akkelpur', 'Kalai', 'Panchbibi'],
    status: 'active',
    flowchart: 'https://example.com/joypurhat-flowchart.png',
    longitude: 89.0412,
    latitude: 25.0953,
    cinemas: ['Joypurhat Star', 'Panchbibi Cinema'],
  },
  {
    region: 'Rajshahi',
    district: 'Bogura',
    city: 'Bogura',
    covered_area: [
      'Sariakandi',
      'Sonatola',
      'Gabtali',
      'Sherpur',
      'Shajahanpur',
    ],
    status: 'active',
    flowchart: 'https://example.com/bogura-flowchart.png',
    longitude: 89.37,
    latitude: 24.85,
    cinemas: ['Bogura Cineplex', 'Sherpur Hall', 'Gabtali Star'],
  },
  {
    region: 'Barisal',
    district: 'Barisal',
    city: 'Barisal',
    covered_area: [
      'Band Road',
      'Cox’s Road',
      'Kawnia',
      'Rupatali',
      'Nathullabad',
    ],
    status: 'active',
    flowchart: 'https://example.com/barisal-flowchart.png',
    longitude: 90.3667,
    latitude: 22.7,
    cinemas: ['Barisal Star', 'Rupatali Cineplex', 'Kawnia Hall'],
  },
  {
    region: 'Barisal',
    district: 'Bhola',
    city: 'Bhola',
    covered_area: ['Borhanuddin', 'Tazumuddin', 'Daulatkhan', 'Char Fasson'],
    status: 'active',
    flowchart: 'https://example.com/bhola-flowchart.png',
    longitude: 90.6311,
    latitude: 22.685,
    cinemas: ['Bhola Cine', 'Char Fasson Hall'],
  },
  {
    region: 'Barisal',
    district: 'Patuakhali',
    city: 'Patuakhali',
    covered_area: ['Kalapara', 'Mirzaganj', 'Dashmina', 'Galachipa'],
    status: 'active',
    flowchart: 'https://example.com/patuakhali-flowchart.png',
    longitude: 90.3333,
    latitude: 22.35,
    cinemas: ['Patuakhali Star', 'Galachipa Cinema'],
  },
  {
    region: 'Barisal',
    district: 'Pirojpur',
    city: 'Pirojpur',
    covered_area: ['Mathbaria', 'Bhandaria', 'Kawkhali', 'Nazirpur'],
    status: 'active',
    flowchart: 'https://example.com/pirojpur-flowchart.png',
    longitude: 89.975,
    latitude: 22.5833,
    cinemas: ['Pirojpur Cineplex', 'Mathbaria Hall'],
  },
  {
    region: 'Barisal',
    district: 'Barguna',
    city: 'Barguna',
    covered_area: ['Amtali', 'Patharghata', 'Betagi', 'Bamna'],
    status: 'active',
    flowchart: 'https://example.com/barguna-flowchart.png',
    longitude: 90.1167,
    latitude: 22.1667,
    cinemas: ['Barguna Star', 'Amtali Cinema'],
  },
  {
    region: 'Barisal',
    district: 'Jhalokati',
    city: 'Jhalokati',
    covered_area: ['Nalchity', 'Rajapur', 'Kathalia'],
    status: 'active',
    flowchart: 'https://example.com/jhalokati-flowchart.png',
    longitude: 90.2167,
    latitude: 22.6417,
    cinemas: ['Jhalokati Cine', 'Nalchity Hall'],
  },
  {
    region: 'Mymensingh',
    district: 'Mymensingh',
    city: 'Mymensingh',
    covered_area: ['Trishal', 'Muktagachha', 'Bhaluka', 'Phulpur', 'Haluaghat'],
    status: 'active',
    flowchart: 'https://example.com/mymensingh-flowchart.png',
    longitude: 90.3987,
    latitude: 24.7539,
    cinemas: ['Mymensingh Star', 'Trishal Cineplex', 'Bhaluka Hall'],
  },
  {
    region: 'Mymensingh',
    district: 'Netrokona',
    city: 'Netrokona',
    covered_area: ['Khaliajuri', 'Mohanganj', 'Durgapur', 'Barhatta'],
    status: 'active',
    flowchart: 'https://example.com/netrokona-flowchart.png',
    longitude: 90.7333,
    latitude: 24.8833,
    cinemas: ['Netrokona Cine', 'Durgapur Hall'],
  },
  {
    region: 'Mymensingh',
    district: 'Jamalpur',
    city: 'Jamalpur',
    covered_area: ['Madarganj', 'Islampur', 'Sarishabari', 'Baksiganj'],
    status: 'active',
    flowchart: 'https://example.com/jamalpur-flowchart.png',
    longitude: 89.9333,
    latitude: 24.9167,
    cinemas: ['Jamalpur Star', 'Sarishabari Cinema'],
  },
  {
    region: 'Mymensingh',
    district: 'Sherpur',
    city: 'Sherpur',
    covered_area: ['Nakla', 'Nalitabari', 'Jhenaigati', 'Sreebardi'],
    status: 'active',
    flowchart: 'https://example.com/sherpur-flowchart.png',
    longitude: 90.0333,
    latitude: 25.0333,
    cinemas: ['Sherpur Cineplex', 'Nakla Hall'],
  },
  {
    region: 'Chattogram',
    district: 'Bandarban',
    city: 'Bandarban',
    covered_area: ['Bandarban Sadar', 'Thanchi', 'Lama', 'Rowangchhari'],
    status: 'active',
    flowchart: 'https://example.com/bandarban-flowchart.png',
    longitude: 92.2186,
    latitude: 22.1958,
    cinemas: ['Bandarban Star', 'Lama Cinema'],
  },
  {
    region: 'Khulna',
    district: 'Kushtia',
    city: 'Kushtia',
    covered_area: [
      'Kushtia Sadar',
      'Kumarkhali',
      'Khoksa',
      'Mirpur',
      'Bheramara',
      'Daulatpur',
    ],
    status: 'active',
    flowchart: 'https://example.com/kushtia-flowchart.png',
    longitude: 89.122,
    latitude: 23.9013,
    cinemas: ['Kushtia Cineplex', 'Kumarkhali Hall', 'Mirpur Star'],
  },
]
module.exports = { locations }; export default router;